...
func TestTransformTrail(t *testing.T) {
	...
	t.Run("empty attestations_statuses array becomes empty map", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": []any{},
			},
		}
		result := TransformTrail(input)
		resultMap := result.(map[string]any)
		cs := resultMap["compliance_status"].(map[string]any)
		as := cs["attestations_statuses"]
		require.IsType(t, map[string]any{}, as)
		assert.Empty(t, as)
	})
	...
	t.Run("single trail-level attestation becomes map entry keyed by name", func(t *testing.T) {
		...
		resultMap := result.(map[string]any)
		cs := resultMap["compliance_status"].(map[string]any)
		as := cs["attestations_statuses"].(map[string]any)
		bar := as["bar"].(map[string]any)
	...
	t.Run("multiple trail-level attestations all present in map", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": []any{
					map[string]any{"attestation_name": "alpha"},
					map[string]any{"attestation_name": "beta"},
					map[string]any{"attestation_name": "gamma"},
				},
			},
		}
		result := TransformTrail(input)
		cs := result.(map[string]any)["compliance_status"].(map[string]any)
		as := cs["attestations_statuses"].(map[string]any)
		assert.Len(t, as, 3)
	...
	t.Run("artifact-level attestations_statuses array becomes map", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"artifacts_statuses": map[string]any{
					"cli": map[string]any{
						"attestations_statuses": []any{
							map[string]any{"attestation_name": "foo", "is_compliant": true},
						},
					},
				},
			},
		...
		result := TransformTrail(input)
		cs := result.(map[string]any)["compliance_status"].(map[string]any)
		cli := cs["artifacts_statuses"].(map[string]any)["cli"].(map[string]any)
		as := cli["attestations_statuses"].(map[string]any)
		foo := as["foo"].(map[string]any)
	...
	t.Run("both trail-level and artifact-level transform in one call", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": []any{
					map[string]any{"attestation_name": "trail-att"},
				},
				"artifacts_statuses": map[string]any{
					"art1": map[string]any{
						"attestations_statuses": []any{
							map[string]any{"attestation_name": "art-att"},
						},
					},
				},
			},
		...
		result := TransformTrail(input)
		cs := result.(map[string]any)["compliance_status"].(map[string]any)
		trailAs := cs["attestations_statuses"].(map[string]any)
		assert.Contains(t, trailAs, "trail-att")
		art1 := cs["artifacts_statuses"].(map[string]any)["art1"].(map[string]any)
		artAs := art1["attestations_statuses"].(map[string]any)
	...
	t.Run("multiple artifacts each get their attestations transformed", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"artifacts_statuses": map[string]any{
					"art1": map[string]any{
						"attestations_statuses": []any{
							map[string]any{"attestation_name": "a1"},
						},
					},
					"art2": map[string]any{
						"attestations_statuses": []any{
							map[string]any{"attestation_name": "a2"},
						},
					},
				},
			},
		...
		result := TransformTrail(input)
		cs := result.(map[string]any)["compliance_status"].(map[string]any)
		arts := cs["artifacts_statuses"].(map[string]any)
		art1As := arts["art1"].(map[string]any)["attestations_statuses"].(map[string]any)
		assert.Contains(t, art1As, "a1")
		art2As := arts["art2"].(map[string]any)["attestations_statuses"].(map[string]any)
	...
	t.Run("entry without attestation_name is skipped", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": []any{
					map[string]any{"attestation_name": "good"},
					map[string]any{"something_else": "no name"},
				},
			},
		}
		result := TransformTrail(input)
...
func TestFilterAttestations(t *testing.T) {
	...
	t.Run("plain name removes non-matching trail-level attestations", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": map[string]any{
					"alpha": map[string]any{"attestation_name": "alpha"},
					"beta":  map[string]any{"attestation_name": "beta"},
					"gamma": map[string]any{"attestation_name": "gamma"},
				},
			},
		}
	...
	t.Run("dot-qualified name keeps only matching artifact-level attestation", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"artifacts_statuses": map[string]any{
					"cli": map[string]any{
						"attestations_statuses": map[string]any{
							"foo": map[string]any{"attestation_name": "foo"},
							"bar": map[string]any{"attestation_name": "bar"},
						},
					},
				},
			},
		...
		cli := result.(map[string]any)["compliance_status"].(map[string]any)["artifacts_statuses"].(map[string]any)["cli"].(map[string]any)
		as := cli["attestations_statuses"].(map[string]any)
		assert.Len(t, as, 1)
	...
	t.Run("dot-qualified name: unmentioned artifact gets empty attestations_statuses", func(t *testing.T) {
		...
		result := FilterAttestations(input, []string{"cli.foo"})
		arts := result.(map[string]any)["compliance_status"].(map[string]any)["artifacts_statuses"].(map[string]any)
		// cli keeps foo
		cliAs := arts["cli"].(map[string]any)["attestations_statuses"].(map[string]any)
		assert.Len(t, cliAs, 1)
		assert.Contains(t, cliAs, "foo")
		// server gets empty attestations_statuses
		serverAs := arts["server"].(map[string]any)["attestations_statuses"].(map[string]any)
	...
	t.Run("mixed filters: trail-level and artifact-level both applied", func(t *testing.T) {
		input := map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": map[string]any{
					"trail-att":   map[string]any{"attestation_name": "trail-att"},
					"trail-other": map[string]any{"attestation_name": "trail-other"},
				},
				"artifacts_statuses": map[string]any{
					"cli": map[string]any{
						"attestations_statuses": map[string]any{
							"art-att":   map[string]any{"attestation_name": "art-att"},
							"art-other": map[string]any{"attestation_name": "art-other"},
						},
					},
				},
			},
		...
		assert.Contains(t, trailAs, "trail-att")
		cliAs := cs["artifacts_statuses"].(map[string]any)["cli"].(map[string]any)["attestations_statuses"].(map[string]any)
		assert.Len(t, cliAs, 1)
	...
	makeInput := func() any {
		return map[string]any{
			"compliance_status": map[string]any{
				"attestations_statuses": map[string]any{
					"bar": map[string]any{"attestation_name": "bar"},
					"baz": map[string]any{"attestation_name": "baz"},
				},
				"artifacts_statuses": map[string]any{
					"cli": map[string]any{
						"attestations_statuses": map[string]any{
...
func TestRehydrateTrail(t *testing.T) {
	t.Run("nil details map leaves trail unchanged", func(t *testing.T) {
		...
		resultMap := result.(map[string]any)
		cs := resultMap["compliance_status"].(map[string]any)
		bar := cs["attestations_statuses"].(map[string]any)["bar"].(map[string]any)
		assert.Equal(t, "bar", bar["attestation_name"])
	...
	t.Run("empty details map leaves trail unchanged", func(t *testing.T) {
		...
		result := RehydrateTrail(input, map[string]any{})
		bar := result.(map[string]any)["compliance_status"].(map[string]any)["attestations_statuses"].(map[string]any)["bar"].(map[string]any)
		assert.Equal(t, "bar", bar["attestation_name"])
		assert.Nil(t, bar["origin_url"])
	})
	...
	t.Run("merges detail fields into artifact-level attestation", func(t *testing.T) {
		...
		details := map[string]any{
			"att-uuid-002": map[string]any{
				"origin_url": "https://example.com/artifact",
			},
		}
		result := RehydrateTrail(input, details)
		foo := result.(map[string]any)["compliance_status"].(map[string]any)["artifacts_statuses"].(map[string]any)["cli"].(map[string]any)["attestations_statuses"].(map[string]any)["foo"].(map[string]any)
