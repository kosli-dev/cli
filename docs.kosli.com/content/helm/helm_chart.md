...
```shell
helm repo add kosli https://charts.merkely.com/
helm repo update
...
| image.repository | string | `"ghcr.io/kosli-dev/merkely-cli"` | the kosli reporter image repository |
| image.tag | string | `"v1.1.0"` | the kosli reporter image tag, overrides the image tag whose default is the chart appVersion. |
| kosliApiToken.secretKey | string | `""` | the name of the key in the secret data which containts the kosli API token |
