...
- The work is planned as ten slices with the plan committed to the repository before any code, because the ticket's hard part is the contract between two evaluators that disagree, and that had to be established against the API rather than assumed.
...
- This repository's test environment cannot complete a server-side evaluation at all: the local stack runs a server, a database and object storage, with no queue broker, no worker and no evaluator. The local server does not exercise the entitlement check either, so the refusal an unentitled organisation would get cannot be reproduced there. Every test of the new path therefore drives a stubbed HTTP server, and the existing tests stay on the real local server to prove the unflagged path is untouched. The cost accepted is that these tests pin our side of the contract only; real agreement between the two paths has to be measured on staging.
- ~~A denial, a broken policy, an expired wait and a fault of ours become four distinguishable exit codes, which this CLI has never had.~~ (superseded by the decision below: every failure keeps the one exit code this CLI has always used.)
- Adding a flag to this CLI costs more than the flag. A repository-wide test pins every command and flag to an audit that runs each one with its value emptied, so a new flag has to be added in two places beyond the command itself: a fixture generated from the command tree, and the audit's own specification. Missing either leaves the build red or the audit refusing to run. The flag is registered with a value of false in the audit rather than true, because the audit stops any run that waits more than five seconds and a genuine server-side evaluation needs the queue, worker and evaluator that the audit's server does not have. That makes the audit exercise the flag's parsing and not its behaviour, which is the same gap as everything else waiting on a fuller test stack.
