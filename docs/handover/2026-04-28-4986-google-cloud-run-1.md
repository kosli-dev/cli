...
- Test stub fixtures (`stubServices` in `cmd/kosli/snapshotCloudRun_test.go`) use full 64-char hex digests, not short placeholders. Why: the server's `CloudRunReport` Pydantic model validates `^[a-f0-9]{64}$|''` per digest value, so the integration test would 422 with the previous `"aaa"`/`"bbb"` fixtures.
