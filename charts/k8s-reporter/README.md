...
| reporterConfig.environments | list | `[]` | **Required.** List of environments to report to. Each entry: `name` (required), optional `namespaces`, `namespacesRegex`, `excludeNamespaces`, `excludeNamespacesRegex`. One entry = one environment; multiple entries = multiple environments with different selectors. |
