...
| reporterConfig.includeScaling | string | `nil` (server default) | whether to record scaling (replica count) changes for auto-created environments. Leave unset to use the Kosli server default; set to `true` to pass `--include-scaling` or `false` to pass `--exclude-scaling`. Only used when `autoEnvironment` is true. |
