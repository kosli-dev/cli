...
| env | object | `{}` | map of plain environment variables to inject into the reporter container. For value-only key=value pairs. Use `extraEnvVars` below if you need `valueFrom` (secretKeyRef / configMapKeyRef) or any other structured env entry. Example for a single tenant Kosli instance:   env:     KOSLI_HOST: https://<instance_name>.kosli.com |
