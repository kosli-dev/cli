...
func (suite *AttestSnykCommandTestSuite) TestAttestSnykCmd() {
	tests := []cmdTestCase{
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest snyk testdata/file1 --artifact-type file --name foo --commit HEAD --origin-url example.com --scan-results testdata/snyk_sarif.json  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest snyk testdata/file1 --artifact-type file --name bar --commit HEAD --origin-url example.com --scan-results testdata/snyk_sarif.json  %s", suite.defaultKosliArguments),
			golden: "snyk attestation 'bar' is reported to trail: test-123\n",
		},
		{
			name:   "can attest snyk against an artifact using --fingerprint",
			cmd:    fmt.Sprintf("attest snyk --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --commit HEAD --origin-url example.com --scan-results testdata/snyk_sarif.json %s", suite.defaultKosliArguments),
			golden: "snyk attestation 'foo' is reported to trail: test-123\n",
		},
		{
			name:   "can attest snyk against a trail",
			cmd:    fmt.Sprintf("attest snyk --name bar --commit HEAD --origin-url example.com --scan-results testdata/snyk_sarif.json %s", suite.defaultKosliArguments),
			golden: "snyk attestation 'bar' is reported to trail: test-123\n",
		},
		{
