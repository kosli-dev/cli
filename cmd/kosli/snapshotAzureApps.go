...
(for example one containing a ^..^ segment) fails the snapshot rather than being written, and the app cannot
be reported until it is redeployed without that entry.
