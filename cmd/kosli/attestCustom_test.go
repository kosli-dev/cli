...
func (suite *AttestCustomCommandTestSuite) SetupTest() {
	...
	suite.artifactFingerprint = "da0c762d2bac7f90d758b5b27509e5b263fa01ccbc542ab5e3df163be08e6ca9"
	...
	suite.defaultKosliArguments = fmt.Sprintf(" --type-name %s --attestation-data %s --flow %s --trail %s --repo-root ../.. --host %s --org %s --api-token %s", suite.typeName, suite.attestationDataFile, suite.flowName, suite.trailName, global.Host, global.Org, global.ApiToken)
...
func (suite *AttestCustomCommandTestSuite) TestAttestCustomCmd() {
	tests := []cmdTestCase{
		...
		//	cmd:       fmt.Sprintf("attest custom testdata/file1 --fingerprint xxxx --artifact-type file --name bar --commit HEAD --origin-url http://example.com  %s", suite.defaultKosliArguments),
