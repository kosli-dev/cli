...
func (suite *AttestOverrideCommandTestSuite) TestAttestOverrideCmd() {
	tests := []cmdTestCase{
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest override --reason r --original-attestation-type generic %s", suite.defaultKosliArguments),
			golden:    "Error: required flag(s) \"name\" not set\n",
		},
		{
			wantError: true,
			name:      "fails when missing required --reason flag",
			cmd:       fmt.Sprintf("attest override --name foo --original-attestation-type generic %s", suite.defaultKosliArguments),
			golden:    "Error: required flag(s) \"reason\" not set\n",
		},
		{
			wantError: true,
			name:      "fails when missing required --original-attestation-type flag",
			cmd:       fmt.Sprintf("attest override --name foo --reason r %s", suite.defaultKosliArguments),
			golden:    "Error: required flag(s) \"original-attestation-type\" not set\n",
		},
		{
			wantError: true,
			name:      "fails when --name is passed as empty string",
			cmd:       fmt.Sprintf("attest override --name \"\" --reason r --original-attestation-type generic %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest override testdata/file1 --fingerprint xxxx --artifact-type file --name bar --reason r --original-attestation-type generic %s", suite.defaultKosliArguments),
			golden:    "Error: only one of --fingerprint, --artifact-type is allowed\n",
		},
		{
			wantError: true,
			name:      "fails when --fingerprint is not valid",
			cmd:       fmt.Sprintf("attest override --name foo --fingerprint xxxx --reason r --original-attestation-type generic %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest override --name foo --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --reason r --original-attestation-type generic --repo-url not-a-url %s", suite.defaultKosliArguments),
			golden:    "Error: --repo-url 'not-a-url' is not a valid URL\n",
		},
		{
			wantError: true,
			name:      "fails when --repo-provider is not an allowed value",
			cmd:       fmt.Sprintf("attest override --name foo --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --reason r --original-attestation-type generic --repo-provider jenkins %s", suite.defaultKosliArguments),
			golden:    "Error: --repo-provider 'jenkins' is not allowed. Must be one of: github, gitlab, bitbucket, azure-devops\n",
		},
		{
			name:   "can override an attestation against a trail",
			cmd:    fmt.Sprintf("attest override --name bar --reason 'manual review' --original-attestation-type generic %s", suite.defaultKosliArguments),
			golden: "attestation 'bar' has been overridden in trail: test-123\n",
		},
		{
			name:   "can override an attestation against a trail with new-compliance-status=false",
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest override --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --reason 'approved out of band' --original-attestation-type generic %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest override --name bar --reason r --original-attestation-type generic --description 'some desc' --annotate foo=bar --external-url ref=https://example.com %s", suite.defaultKosliArguments),
			golden: "attestation 'bar' has been overridden in trail: test-123\n",
		},
		{
			wantError: true,
			name:      "fails when annotation key is invalid",
			cmd:       fmt.Sprintf("attest override --name bar --reason r --original-attestation-type generic --annotate foo.baz=bar %s", suite.defaultKosliArguments),
