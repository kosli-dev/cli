...
func (suite *AttestGitlabPRCommandTestSuite) TestAttestGitlabPRCmd() {
	tests := []cmdTestCase{
		...
		},
		{
			...
			golden:    "Error: required flag(s) \"commit\", \"gitlab-org\", \"name\", \"repository\" not set\n",
		},
		{
			wantError: true,
			name:      "fails when both --fingerprint and --artifact-type",
			cmd:       fmt.Sprintf("attest pullrequest gitlab testdata/file1 --fingerprint xxxx --artifact-type file --name bar --commit HEAD  %s", suite.defaultKosliArguments),
			golden:    "Error: only one of --fingerprint, --artifact-type is allowed\n",
		},
		{
			wantError: true,
			name:      "fails when --fingerprint is not valid",
			cmd:       fmt.Sprintf("attest pullrequest gitlab --name foo --fingerprint xxxx --commit HEAD %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd: fmt.Sprintf(`attest pullrequest gitlab --fingerprint 1234e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo 
						--gitlab-org ewelinawilkosz --repository merkely-gitlab-demo --commit HEAD  %s`, suite.defaultKosliArguments),
