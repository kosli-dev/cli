...
func (suite *AttestSonarQubeCommandTestSuite) TestAttestSonarQubeCmd() {
	tests := []cmdTestCase{
		{
			wantError: true,
			name:      "100 fails when more arguments are provided",
		...
		{
			wantError: true,
			name:      "101 fails when both --fingerprint and --artifact-type",
		...
		{
			wantError: true,
			name:      "102 fails when --fingerprint is not valid",
		...
		{
			wantError: true,
			name:      "103 attesting against an artifact that does not exist fails",
		...
		{
			name:   "104 can attest sonar against an artifact using artifact name and --artifact-type",
		...
		{
			name:   "105 can attest sonar against an artifact using artifact name and --artifact-type when --name does not exist in the trail template",
		...
		{
			name:   "106 can attest sonar against an artifact using --fingerprint",
		...
		{
			name:   "107 can attest sonar against a trail",
		...
		{
			name:   "108 can attest sonar against a trail when name is not found in the trail template",
		...
		{
			name:   "109 can attest sonar against an artifact it is created using dot syntax in --name",
		...
		{
			wantError: true,
			name:      "111 if no path to the scannerwork directory is provided and the command is not being run in the same base directory, we get an error",
			cmd:       fmt.Sprintf("attest sonar --name cli.foo --commit HEAD --origin-url http://www.example.com %s", suite.defaultKosliArguments),
			golden:    "Error: open .scannerwork/report-task.txt: no such file or directory. Check your working directory is set correctly. Alternatively provide the project key and either revision or pull-request ID for the scan to attest\n",
		},
		{
			name:   "112 can retrieve scan results using project key and revision and attest them",
		...
		{
			wantError: true,
			name:      "114 if incorrect project key given, we get an error message from Sonar",
		...
		{
			name:   "117 can retrieve scan results using report-task.txt file and pull-request flag",
		...
		{
			wantError: true,
			name:      "118 fails when --name has invalid dot format",
			cmd:       fmt.Sprintf("attest sonar --name .foo %s", suite.defaultKosliArguments),
			golden:    "Error: failed to parse attestation name: invalid attestation name format: .foo\n",
		},
	}
