...
func (suite *AttestGenericCommandTestSuite) TestAttestGenericCmd() {
	tests := []cmdTestCase{
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest generic testdata/file1 --fingerprint xxxx --artifact-type file --name bar --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
			golden:    "Error: only one of --fingerprint, --artifact-type is allowed\n",
		},
		{
			wantError: true,
			name:      "fails when --fingerprint is not valid",
			cmd:       fmt.Sprintf("attest generic --name foo --fingerprint xxxx --commit HEAD --build-url example.com %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest generic --fingerprint 1234e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest generic testdata/file1 --artifact-type file --name foo --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest generic testdata/file1 --artifact-type file --name bar --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest generic --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
			golden: "generic attestation 'foo' is reported to trail: test-123\n",
		},
		{
			name:   "can attest generic against a trail",
			cmd:    fmt.Sprintf("attest generic --name bar --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
			golden: "generic attestation 'bar' is reported to trail: test-123\n",
		},
		{
			name:   "can attest generic against a trail when name is not found in the trail template",
			cmd:    fmt.Sprintf("attest generic --name additional --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest generic --name cli.foo --commit HEAD --build-url example.com  %s", suite.defaultKosliArguments),
			golden: "generic attestation 'foo' is reported to trail: test-123\n",
		},
		{
