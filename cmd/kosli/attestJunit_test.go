...
func (suite *AttestJunitCommandTestSuite) TestAttestJunitCmd() {
	tests := []cmdTestCase{
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest junit testdata/file1 --fingerprint xxxx --artifact-type file --name bar --commit HEAD --url example.com  %s", suite.defaultKosliArguments),
			golden:    "Error: only one of --fingerprint, --artifact-type is allowed\n",
		},
		{
			wantError: true,
			name:      "fails when --fingerprint is not valid",
			cmd:       fmt.Sprintf("attest junit --name foo --fingerprint xxxx --commit HEAD --url example.com %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:       fmt.Sprintf("attest junit --fingerprint 1234e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --commit HEAD --url example.com --results-dir testdata  %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest junit testdata/file1 --artifact-type file --name foo --commit HEAD --url example.com --results-dir testdata %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest junit testdata/file1 --artifact-type file --name bar --commit HEAD --url example.com --results-dir testdata  %s", suite.defaultKosliArguments),
			golden: "junit attestation 'bar' is reported to trail: test-123\n",
		},
		{
			name:   "can attest junit against an artifact using --fingerprint",
			cmd:    fmt.Sprintf("attest junit --fingerprint 7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9 --name foo --commit HEAD --url example.com --results-dir testdata %s", suite.defaultKosliArguments),
			golden: "junit attestation 'foo' is reported to trail: test-123\n",
		},
		{
			name:   "can attest junit against a trail",
			cmd:    fmt.Sprintf("attest junit --name bar --commit HEAD --url example.com --results-dir testdata %s", suite.defaultKosliArguments),
			golden: "junit attestation 'bar' is reported to trail: test-123\n",
		},
		{
			name:   "can attest junit against a trail when name is not found in the trail template",
			cmd:    fmt.Sprintf("attest junit --name additional --commit HEAD --url example.com --results-dir testdata %s", suite.defaultKosliArguments),
		...
		},
		{
			...
			cmd:    fmt.Sprintf("attest junit --name cli.foo --commit HEAD --url example.com --results-dir testdata %s", suite.defaultKosliArguments),
			golden: "junit attestation 'foo' is reported to trail: test-123\n",
		},
	}
