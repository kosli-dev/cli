...
Paths the list already matches stay excluded whatever is later added there, so keep its entries as narrow as possible.`
	...
	policyDecisionFlowFlag          = "[conditional] The Kosli flow the decision is recorded in. Only required with --control."
	policyDecisionTrailFlag         = "[conditional] The Kosli trail the decision is recorded in. Only required with --control."
	policyDecisionNameFlag          = "[defaulted] The attestation name the decision is recorded under. Defaults to <control>-decision."
	policyDecisionFingerprintFlag   = "[optional] The SHA256 fingerprint of the artifact the decision is about. Without it the decision is about the trail."
	...
	trailUserDataFlag               = "[optional] The path to a JSON file containing additional data you would like to attach to the flow trail. The maximum JSON payload size is 1MB."
	...
	commitUrlFlag                   = "The url for the git commit that created the artifact. (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd )."
	...
	repoURLFlag                     = "[conditional] The URL of the repository. Must be a valid URL. All three of --repo-id, --repo-url and --repository must be set to record repository information (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd )."
	repoProviderFlag                = "[optional] The source code hosting provider. One of: " + repoProviderList + " (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd )."
	bbUsernameFlag                  = "Bitbucket username (your Atlassian account email when using an API token). Only needed if you use --bitbucket-password"
	bbPasswordFlag                  = "Bitbucket API token. Bitbucket app passwords are no longer supported as of 28 July 2026. See [Bitbucket authentication](https://developer.atlassian.com/cloud/bitbucket/rest/intro/#authentication) for more details."
	bbAccessTokenFlag               = "Bitbucket repo/project/workspace access token. See [Bitbucket access tokens](https://developer.atlassian.com/cloud/bitbucket/rest/intro/#access-tokens) for more details."
	bbWorkspaceFlag                 = "Bitbucket workspace ID."
	commitPREvidenceFlag            = "Git commit for which to find pull request evidence. (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd )."
	repositoryFlag                  = "Git repository. (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd )."
	...
	azureProjectFlag                = "Azure project.(defaulted if you are running in Azure Devops pipelines: https://docs.kosli.com/integrations/ci_cd )."
	azureOrgUrlFlag                 = "Azure organization url. E.g. 'https://dev.azure.com/myOrg' (defaulted if you are running in Azure Devops pipelines: https://docs.kosli.com/integrations/ci_cd )."
	...
	ecsExcludeClustersFlag          = "[optional] The comma-separated list of ECS cluster names to exclude. Can't be used together with --clusters or --clusters-regex."
	...
	ecsServiceFlag                  = "[deprecated] The name of the ECS service. Use --services instead."
	ecsServicesFlag                 = "[optional] The comma-separated list of ECS service names to snapshot. Can't be used together with --exclude-services or --exclude-services-regex."
	...
	cloudRunExcludeRegexFlag        = "[optional] The comma-separated list of Cloud Run service or job name regex patterns to exclude. Can't be used together with --include or --include-regex."
	...
	excludeRegexFlag                = "[optional] The comma-separated list of name regex patterns for AWS Lambda functions to be excluded. Cannot be used together with --function-names. Allowed regex patterns are described in [RE2 syntax](https://github.com/google/re2/wiki/Syntax)"
	...
	beginTrailCommitFlag            = "[defaulted] The git commit from which the trail is begun. (defaulted in some CIs: https://docs.kosli.com/integrations/ci_cd, otherwise unset ). If both --commit and --repo-root are left at their defaults and the commit cannot be read from the repository, a warning is printed and the trail is begun without commit info."
	...
	sonarPRFlag                     = "[conditional] The ID of the pull-request. Only required if you want to use the project key/pull-request to get the scan results rather than using Sonar's metadata file. Cannot be used with --sonar-branch."
	...
	attestationTypeSummaryFlag      = "[optional] An attestation type summary entry, given as 'NAME=EXPRESSION'. Can be repeated. Cannot be used with --summary-json."
	attestationTypeSummaryJsonFlag  = "[optional] The attestation type summary, given as a JSON array of {name, expression} entries, e.g. '[{\"name\":\"Critical\",\"expression\":\".critical_count\"}]'. Cannot be used with --summary."
