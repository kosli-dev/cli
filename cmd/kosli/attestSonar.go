...
the scan results are not available by the time the attest sonar command is executed. In this case you can use the ^--allow-wait^ flag to wait for the scan to be processed. 
This flag takes the maximum number of seconds to wait, and allows the Kosli CLI to periodically poll SonarQube to check if the scan results are available. Once the results 
...
func newAttestSonarCmd(out io.Writer) *cobra.Command {
	...
	cmd.Flags().IntVar(&o.allowWait, "allow-wait", 0, sonarWaitFlag)
