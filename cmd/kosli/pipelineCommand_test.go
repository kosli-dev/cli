...
func (suite *PipelineCommandTestSuite) TestPipelineCommandCmd() {
	...
	defaultArtifactArguments := " --flow newFlow --build-url https://www.yr.no --commit-url https://www.nrk.no"
	...
	tests := []cmdTestCase{
		...
		},
		{
			...
			goldenRegex: "Error: open thisIsNotAFile: no such file or directory\n",
		},
		{
			wantError:   true,
